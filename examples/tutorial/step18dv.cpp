...
//
void
TDrawView::EvSize(UINT SizeType, const TSize& Size)
{
...
//
bool
TDrawView::Select(uint modKeys, const TPoint& point)
{
  if (Tool != DrawSelect)
    return false;
...
void
TDrawView::EvLButtonDown(uint modKeys, const TPoint& point)
{
...
void
TDrawView::EvMouseMove(uint modKeys, const TPoint& point)
{
...
TDrawView::CeControlProps(TCommandEnabler& ce)
{
  ...
  ocf:: TOcPart* p = OcView->GetActivePart();
  if (p) {
    IBControl *CtrlI;
...
TDrawView::CmNewControl()
{
  ...
  if (OcApp->BrowseControls(initInfo)) {
    ...
    ocf:: TOcControl *ctrl = new ocf:: TOcControl(*GetOcDoc(), 0, (TOcControlEvent *)pEv);
