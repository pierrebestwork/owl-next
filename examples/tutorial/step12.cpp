...
TDrawApp::InitMainWindow()
{
  ...
  TDecoratedFrame* frame = new TDecoratedFrame(0, _T("Drawing Pad"), 0, true);
  ...
  SetMainWindow(frame);
  GetMainWindow()->SetMenuDescr(TMenuDescr(_T("COMMANDS"),1,0,0,0,0,1));
...
TDrawApp::EvDropFiles(TDropInfo dropInfo)
{
  if (dropInfo.DragQueryFileCount() != 1)
    ::MessageBox(0, _T("Can only drop 1 file in SDI mode"), _T("Drag/Drop Error"), MB_OK);
  else {
...
TDrawApp::EvCloseView(TView& /*view*/)
{
  ...
  GetMainWindow()->SetCaption(_T("Drawing Pad"));
}
