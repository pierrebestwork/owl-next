...
public:
  ...
  void InitMainWindow();
  void InitInstance();
  void Output(tostringstream &ostr);
...
//
void
TMyApp::Output(tostringstream &ostr)
{
  const char *str = ostr.str().c_str(); //Get stream buffer as char*
...
{
  char buf[256];
  tostringstream ostr(buf, sizeof buf);
