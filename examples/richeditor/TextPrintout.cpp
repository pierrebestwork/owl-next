...
TTextPrintout::TTextPrintout(TPrinter* printer, LPCTSTR title, TWindow* window, bool scale)
  : TPrintout(title)
{
  Printer = printer;
  Window = window;
...
void TTextPrintout::BeginPrinting()
{
  TRect clientR;
  BeginPage(clientR);
  ...
  Window->SendMessage(WM_GETMINMAXINFO, 0, reinterpret_cast<TParam2>(&minmaxinfo));
...
void TTextPrintout::BeginPage(TRect& clientR)
{
  TScreenDC screenDC;
  TSize screenRes(screenDC.GetDeviceCaps(LOGPIXELSX), screenDC.GetDeviceCaps(LOGPIXELSY));
  TSize printRes(DC->GetDeviceCaps(LOGPIXELSX), DC->GetDeviceCaps(LOGPIXELSY));
...
void TTextPrintout::PrintPage(int page, TRect& bandRect, unsigned)
{
  TRect clientR;
  BeginPage(clientR);
  if (Scale)
...
void TTextPrintout::SetBanding(bool b)
{
  Banding = b;
}
