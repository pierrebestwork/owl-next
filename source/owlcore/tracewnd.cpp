...
tstring GetDiagIniFullFileName_(LPCSTR filename); // Private function defined in "diaginit.cpp".
...
TGroupList::SetGroupState(int index, bool enabled)
{
  ...
  TProfile iniFile(_T("Diagnostics"), GetDiagIniFullFileName_(OWL_INI));
  tchar buffer[20];
  ...
  Active(true)
{
  ...
  TProfile iniFile(_T("TraceWindow"), GetDiagIniFullFileName_(OWL_INI));
  tchar buffer[40];
  iniFile.GetString(_T("Position"), buffer, 40, _T("-1 -1 -1 -1"));
  TRect rect(-1, -1, -1, -1);
...
TTraceWindow::FormatGroup (TDiagBase* group)
{
  ...
  TProfile iniFile(_T("Diagnostics Descriptions"), GetDiagIniFullFileName_(OWL_INI));
  tchar buffer[80];
...
TTraceWindow::SaveWindowState()
{
  TProfile iniFile(_T("TraceWindow"), GetDiagIniFullFileName_(OWL_INI));
  tchar buffer[40];
  wsprintf(buffer, _T("%d %d %d %d"), Attr.X, Attr.Y, Attr.X + Attr.W, Attr.Y + Attr.H);
...
void TTraceWindow::CmSetLevel ()
{
  ...
  TInputDialog dlg(this, _T("Diagnostic Window"), prompt.c_str(),
                   level, COUNTOF(level), module, new TRangeValidator(INT_MIN, INT_MAX));

  if (dlg.Execute()){
    const int l = _ttoi(level);
    group->SetLevel(l);
...
void TTraceWindow::CmTop ()
{
  ...
  TProfile(_T("TraceWindow"), GetDiagIniFullFileName_(OWL_INI)).WriteInt(_T("TopMost"), topMost ? 1 : 0);
}
