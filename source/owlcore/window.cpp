...
TWindow::EvNotify(uint ctlId, TNotify& notifyInfo)
{
  ...
  {
    TTooltipText& ttText = reinterpret_cast<TTooltipText&>(notifyInfo);
    TTooltipEnabler enabler(ttText, GetHandle());
    const TWindow* const cmdTarget = GetParent() ? GetParent() : this;
    RouteCommandEnable(cmdTarget->GetHandle(), enabler);
    const bool handled = enabler.GetHandled();
...
//
/// Respond to MouseHWheel event if Scroller exist
//
bool
TWindow::EvMouseHWheel(uint modKeys, int zDelta, const TPoint& /*point*/)
{
  ...
  if (Scroller)
  {
    ...
    {
      if (zDelta > 0)
        scrollCnt  = -((int)Scroller->XPage);
      else
        scrollCnt  = ((int)Scroller->XPage);
    }
    else
    {
      scrollCnt = ::MulDiv(-zDelta, uWheelScrollLines, WHEEL_DELTA) * (int)Scroller->XLine;
    }


//    int    scrollCnt = ::MulDiv(-zDelta, uWheelScrollLines, WHEEL_DELTA);
//    scrollCnt = scrollCnt*(int)Scroller->XLine;
//    if (scrollCnt == -1 || uWheelScrollLines == WHEEL_PAGESCROLL){
//      scrollCnt  = (int)Scroller->XPage;
//      if (zDelta > 0)
//        scrollCnt  = -((int)Scroller->XPage);
...
int
TWindow::FormatMessageBox(LPCTSTR text, LPCTSTR caption, uint flags, va_list argp) const
{
...
int
TWindow::FormatMessageBox(LPCTSTR text, LPCTSTR caption, uint flags, ...) const
{
  va_list argp;
  va_start(argp, flags);
  int r = 0;
...
int
TWindow::FormatMessageBox(const tstring& text, const tstring& caption, uint flags, ...) const
{
  va_list argp;
  va_start(argp, flags);
...
int
TWindow::FormatMessageBox(uint resId, LPCTSTR caption, uint flags, va_list argp) const
{
  PRECONDITION(resId);
...
int
TWindow::FormatMessageBox(uint resId, LPCTSTR caption, uint flags, ...) const
{
  va_list argp;
  va_start(argp, flags);
  int r = 0;
...
int
TWindow::FormatMessageBox(uint resId, const tstring& caption, uint flags, ...) const
{
  va_list argp;
  va_start(argp, flags);
  int r = 0;
...
int
TWindow::FormatMessageBox(uint resId, const tstring& caption, uint flags, va_list argp) const
{
  tstring text = LoadString(resId);
