  ...
  TWindowDC()
{
  ...
  // and will destroy it.
  //
  const uint dcxUseStyle = 0x10000u; // Undocumented style.
  const INT_PTR regionCode = reinterpret_cast<INT_PTR>(hPaintRgn);
  if (regionCode == 0 || regionCode == 1)
  {
    Handle = ::GetDCEx(wnd, 0, DCX_WINDOW | DCX_EXCLUDERGN | dcxUseStyle);
  }
  else
  {
