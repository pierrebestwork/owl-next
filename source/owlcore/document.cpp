...
{
// NOTE: Contrary to the documentation, this method short-circuits when any view returns false. See https://sourceforge.net/p/owlnext/bugs/258/
...
///
/// The notification message, OWL_WM_VIEWNOTIFY, is sent with the given eventId, which is
...
//
bool
TDocument::NotifyOwnViews(int event, TParam2 item, TView* exclude)
{
  bool answer = true;
  ...
  for (TView* view = ViewList; view != 0; view = view->NextView)
    if (view != exclude && view->Find(eventInfo))
    {
      bool r = (view->Dispatch(eventInfo, 0, item) != 0);
      answer = answer && r;
    }
