...
TCoolTextWnd::CellImageIndex(int lineIndex)
{
  ...
  if (lineIndex >= 0){
    ...
    for (int i = 0; i <= static_cast<int>(COUNTOF(adwFlags)); i++){
...
#define ALL_FLAGS (TCoolTextBuffer::lfBreakPoint|TCoolTextBuffer::lfCompError|\
  ...
  TCoolTextBuffer::lfReserved2)
      ...
      if((lineFlags&ALL_FLAGS)!=0)
        imageIndex += 9;
      else if(imageIndex < static_cast<int>(COUNTOF(adwFlags)) - 1)
        imageIndex = 18;
    }
  }
  return imageIndex;
...
DirectUp(up)
{
  ...
  for(i = 0; i < static_cast<int>(DeltaSize); i++)
    DeltaTable[i] = patlen;
  ...
  while(true){
    ...
    if (WholeWord && pos){
      ...
      if ((pos > text && _istalnum(pos[-1])) || // Match is in preceding word
        (static_cast<int>(Pattern.length()) < static_cast<int>(_tcslen(pos)) && _istalnum(pos[Pattern.length()])))
      {
          //Jogy:  up -> DirectUp
