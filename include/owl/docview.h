...
/// Destroys a TDocument::TList object.
    ...
    bool           NotifyOwnViews(int event, TParam2 item = 0, TView* exclude = 0);
...
#define VN_DEFINE_STANDARD_(id, method)\
  {{WM_OWLNOTIFY}, id, (::owl::CheckSignature<TMyClass, WM_OWLNOTIFY, id, ::owl::TDispatch>(&TMyClass::method),\
  OWL_DISPATCH(::owl::TDispatch<WM_OWLNOTIFY>::TNotificationDispatch<id>::Decode, method))}
...
//
#define VN_DEFINE_STANDARD_(id, method)\
  {{WM_OWLNOTIFY}, id, OWL_DISPATCH(::owl::TDispatch<WM_OWLNOTIFY>::TNotificationDispatch<id>::Decode, method)}
