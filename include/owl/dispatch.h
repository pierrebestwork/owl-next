...
void CheckSignature(typename D<MsgId>::template THandler<T>::type) {}
...
void CheckSignature(typename D<MsgId>::template TNotificationDispatch<NotificationCode>::template THandler<T>::type) {}
...
template <> struct TDispatch<WM_CHARTOITEM>
{
  ...
  template <class T> struct THandler {typedef int (T::*type)(uint ch, HWND listBox, uint caretIndex);};
...
template <> struct TDispatch<WM_HSCROLL>
{
  ...
  template <class T> struct THandler {typedef void (T::*type)(uint scrollCode, uint thumbPos, HWND ctrl);};
...
template <> struct TDispatch<WM_HSCROLLCLIPBOARD>
{
  ...
  template <class T> struct THandler {typedef void (T::*type)(HWND cliboardViewer, uint scrollCode, uint thumbPos);};
...
template <> struct TDispatch<WM_MOUSEHWHEEL>
{
  ...
  template <class T> struct THandler {typedef bool (T::*type)(uint modKeys, int zDelta, const TPoint&);};
  ...
  static TResult Decode(void* i, TParam1 p1, TParam2 p2)
  {
    ...
    forwarder = {ConvertVoidPtr<T>(i), M};
    InUse(p1); InUse(p2);
    return HANDLE_WM_MOUSEWHEEL(0, p1, p2, forwarder);   // There is no HANDLE_WM_MOUSEHWHEEL in Windows SDK, so reuse HANDLE_WM_MOUSEWHEEL
  }
};
...
template <> struct TDispatch<WM_VKEYTOITEM>
{
  ...
  template <class T> struct THandler {typedef int (T::*type)(uint vk, HWND listBox, uint caretIndex);};

#endif

  template <class T, int (T::*M)(uint vk, HWND listBox, uint caretIndex)>
  static TResult Decode(void* i, TParam1 p1, TParam2 p2)
  {
    struct TForwarder
    {
      ...
      {return (i_->*m)(vk, hwndListbox, static_cast<unsigned int>(iCaret));}
    }
    forwarder = {ConvertVoidPtr<T>(i), M};
    InUse(p1); InUse(p2);
